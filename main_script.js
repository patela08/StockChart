(function iife() {
    ...
    function removeStkFromListAndChart(e) {
        ...
        let stkToRemoveFromArr = qrySymbolsArr.findIndex((stk) => stk == getElementByID(`stkSymbol_${indxToRemove}`).textContent);
        if (cls.indexOf('removeStk') > -1 && stkToRemoveFromArr > -1) {
            qrySymbolsArr.splice(stkToRemoveFromArr, 1);
            dataToDisplay.splice(stkToRemoveFromArr + 1, 1);
            globalVar.listOfStockContainer.removeChild(getElementByID(`stk${indxToRemove}`));
    ...
    function createNewElement(content) {
        ...
        li.innerHTML = `<span id="stkSymbol_${qrySymbolsArr.length}">${content}</span><button class='removeStk_${qrySymbolsArr.length}'>Remove</button>`;
        return li;
    }
